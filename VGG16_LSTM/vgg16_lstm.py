...
class VGG16LSTM(nn.Module):
    def __init__(self, original_model):
        ...
        self.lstm2 = LSTMCell(256,99,True).to(device).float()
        #self.lstm3 = LSTMCell(8,99,True).to(device).float()
    ...
    def forward(self,x,state1,state2):
        ...
        next_state2 = (c2,h2)
        #next_state3 = (c3,h3)
        softmax = F.log_softmax(h2,dim=1)    
...
def train(model,optimizer,train_path,device):
...
def train_model():
    train_path = "/kuacc/users/mustafaakinyilmaz/scratch/UCF_101_Frames/Train/"
    test_path = "/kuacc/users/mustafaakinyilmaz/scratch/UCF_101_Frames/Test/"
    ...
    for epoch in range(1,nb_epoch+1):
        ...
        epoch_train_loss,epoch_train_correct,train_samples = train(vgg16_model,optimizer,train_path,device)
